...
export default function Profil() {
  ...
  const [recipes, setRecipes] = useState<RecipeType[]>(userInfo?.recettes || []);
  ...
  const handleRecipeClick = (recipe: RecipeType) => {
    setSelectedRecipe(recipe); // Mettez à jour la recette sélectionnée
...
console.log(selectedRecipe)
  return (
    <div className="flex flex-col items-center justify-center m-auto w-full mt-10">
     ...
     <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 w-full max-w-5xl m-auto mb-20 my-6'>
      {recipes?.map((recipes)=>(
        <CardRecipesprofile key={recipes.id} titre={recipes.titre} temps={recipes.tps_cook} bgImage={recipes.img} dificulty={recipes.dificulty} onClick={()=>handleRecipeClick(recipes)}/>
      ))}      
      </div>
      {isModalOpen && (
  ...
  <div className="bg-white p-4 rounded max-w-lg shadow-lg max-h-[80vh] overflow-y-auto">
            <CardRecipe 
              ...
              instructions={Array.isArray(selectedRecipe?.instructions) ? selectedRecipe.instructions : []} 
            />
            <button onClick={() => setIsModalOpen(false)} className="mt-4 bg-red-500 text-white p-2 rounded">Fermer</button>
          </div>
        </div>
